...
var
  ...
  space1, space2: array[1 .. HalfMemSize, SExp]
  useSpace1 = true
  currAlloc = 1
...
proc isTrue(s: SExp): bool =
  s != nil and s.kind != skNil

proc initMemory() =
  for i in 1 .. HalfMemSize:
    space1[i] = SExp(moved: false, forwarded: nil, kind: skNil)
    space2[i] = SExp(moved: false, forwarded: nil, kind: skNil)
  useSpace1 = true
  currAlloc = 1

proc gc()

proc allocSExp(k: SExpKind): SExp =
  if currAlloc > HalfMemSize:
    gc()
    if currAlloc > HalfMemSize:
      echo "The garbage collector could not reclaim any more storage."
      raise newException(EvalError, "Memory overflow")
    if currentExp == nil:
      echo "Called gc during parsing. Parse aborted; reenter input"
      raise newException(EvalError, "Parse aborted")
  let s = if useSpace1: space1[currAlloc] else: space2[currAlloc]
  inc currAlloc
  case k
  of skNil: s[] = SExp(moved: false, forwarded: nil, kind: skNil)[]
  of skNum: s[] = SExp(moved: false, forwarded: nil, kind: skNum, intVal: 0)[]
  of skSym: s[] = SExp(moved: false, forwarded: nil, kind: skSym, symVal: "")[]
  of skList: s[] = SExp(moved: false, forwarded: nil, kind: skList, carVal: nil, cdrVal: nil)[]
  s

proc gc() =
  let toSpace1 = not useSpace1
  var toAlloc = 1

  proc copySExp(s: var SExp) =
    if s == nil: return
    if s.moved:
      s = s.forwarded
      return
    let newS = if toSpace1: space1[toAlloc] else: space2[toAlloc]
    inc toAlloc
    newS[] = s[]
    newS.moved = false
    newS.forwarded = nil
  ...
  stdout.write("Switching memories...")
  # Copy roots
  copySExp(nilValue)
  copySExp(trueValue)
  for k in globalEnv.keys:
    var v = globalEnv[k]
    copySExp(v)
    globalEnv[k] = v
  if currentExp != nil:
  ...
  for _, fd in fundefs:
    copyExp(fd.body)
  for i in 0 ..< evalStack.len:
    copySExp(evalStack[i])

  # Cheney scan of to-space
  var scan = 1
  while scan < toAlloc:
    let curr = if toSpace1: space1[scan] else: space2[scan]
    if curr.kind == skList:
      copySExp(curr.carVal)
      copySExp(curr.cdrVal)
    inc scan

  useSpace1 = toSpace1
  currAlloc = toAlloc
  let recovered = HalfMemSize - toAlloc + 1
  echo "recovered " & $recovered & "(" & $(round(100 * recovered / HalfMemSize).int) & "%) cells"

proc mkNum(n: int): SExp =
  let s = allocSExp(skNum)
  s.intVal = n
  s

proc mkSym(name: string): SExp =
  let s = allocSExp(skSym)
  s.symVal = name
  s

proc mkList(car, cdr: SExp): SExp =
  let s = allocSExp(skList)
  s.carVal = car
  s.cdrVal = cdr
  s
...
proc parseSExp(p: var Parser): SExp =
  ...
  of tkLParen:
    ...
    var list = allocSExp(skNil)
    for i in countdown(items.len - 1, 0):
      list = mkList(items[i], list)
    list
  else:
...
proc parseExp(p: var Parser): Exp =
  ...
  of tkInt:
    discard p.next()
    Exp(kind: ekVal, sxp: mkNum(tok.intVal))
  of tkSym:
    discard p.next()
...
proc applyValueOp(op: string, numArgs: int) =
  ...
  let s1 = evalStack[^numArgs]
  let s2 = if numArgs == 2: evalStack[^1] else: nil

  var res: SExp = nilValue
  case op
  of "+", "-", "*", "/":
  ...
  of "<", ">":
    ...
    res = if cond: trueValue else: nilValue
  of "=":
    var eq = false
    if s1.kind == skNil and s2.kind == skNil:
      ...
      eq = true
    res = if eq: trueValue else: nilValue
  of "cons":
    res = mkList(s1, s2)
  of "car":
    if s1.kind != skList:
  ...
  of "cdr":
    if s1.kind != skList:
      ...
      res = s1.cdrVal
  of "number?":
    res = if s1.kind == skNum: trueValue else: nilValue
  of "symbol?":
    res = if s1.kind == skSym: trueValue else: nilValue
  of "list?":
    res = if s1.kind == skList: trueValue else: nilValue
  of "null?":
    res = if s1.kind == skNil: trueValue else: nilValue
  of "print":
    echo $s1
    res = s1

  # Pop arguments and push result
  evalStack.setLen(evalStack.len - numArgs)
  evalStack.add(res)
...
proc applyUserFun(fn: FunDef, numArgs: int) =
  ...
  for i in 0 ..< numArgs:
    actuals[i] = evalStack[evalStack.len - numArgs + i]
