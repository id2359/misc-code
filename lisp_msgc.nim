# Modern Idiomatic Nim: Lisp with Mark-Scan Garbage Collector
...
proc initMemory() =
  for i in 1 .. MemSize:
    memory[i] = SExp(mark: false, kind: skList, carVal: nil, cdrVal: nil)
  for i in 1 ..< MemSize:
    memory[i].carVal = memory[i + 1]
...
proc allocSExp(k: SExpKind): SExp =
  ...
  freeSEXP = s.carVal
  case k
  of skNil: s[] = SExp(mark: false, kind: skNil)[]
  of skNum: s[] = SExp(mark: false, kind: skNum, intVal: 0)[]
  of skSym: s[] = SExp(mark: false, kind: skSym, symVal: "")[]
  of skList: s[] = SExp(mark: false, kind: skList, carVal: nil, cdrVal: nil)[]
  s
...
proc gc() =
  ...
  proc markPhase() =
    proc markSExp(s: SExp) =
      if s != nil and not s.mark:
        s.mark = true
        if s.kind == skList:
          markSExp(s.carVal)
          markSExp(s.cdrVal)
  ...
  proc scanPhase() =
    ...
    collected = 0
    for i in 1 .. MemSize:
      if not memory[i].mark:
        memory[i][] = SExp(mark: false, kind: skList, carVal: freeSEXP, cdrVal: nil)[]
        freeSEXP = memory[i]
        inc collected

  stdout.write("Garbage collection ...")
  unmarkPhase()
  markPhase()
  scanPhase()
  echo "collected " & $collected & "(" & $(round(100 * collected / MemSize).int) & "%) records"
