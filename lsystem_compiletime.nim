...
# Shows Nim macros, static:, and compile-time execution
# nim c -r lsystem_compiletime.nim

import math, strformat, tables, os
...
proc turtle(commands: string, step: float64, turnDeg: float64): seq[Segment] =
  ...
  var state = TurtleState(x:0,y:0,ang:90) # up
  for c in commands:
    case c
    of 'F','G':
...
proc toSVG(segs: seq[Segment], filename: string) =
  ...
  var svg = &"<svg xmlns='http://www.w3.org/2000/svg' width='{w:.0f}' height='{h:.0f}' viewBox='{minX-pad} {minY-pad} {w} {h}'>\n"
  svg.add "<path d='"
  for s in segs:
    svg.add &"M {s.x1:.2f} {s.y1:.2f} L {s.x2:.2f} {s.y2:.2f} "
  svg.add "' stroke='black' fill='none' stroke-width='0.8'/>\n</svg>"
