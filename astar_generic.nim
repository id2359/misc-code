# astar_generic.nim
# Generic A* with compile-time interface verification, heapqueue, and grid adapters
# nim c -r astar_generic.nim

import algorithm, heapqueue, tables, math, sequtils, sets, strformat, strutils
...
iterator neighbors8*(g: Grid, p: GridPos): GridPos =
  for dy in -1..1:
    for dx in -1..1:
      ...
      # Prevent corner cutting when squeezing between two orthogonal walls
      if dx != 0 and dy != 0:
...
# Adapter API used by generic graph search algorithms
proc neighbors*(g: Grid, p: GridPos): seq[GridPos] =
  for np in g.neighbors8(p):
    result.add np

func cost*(g: Grid, source, target: GridPos): float =
  discard g
  if abs(source.x - target.x) + abs(source.y - target.y) == 2: Sqrt2 else: 1.0

func heuristic*(g: Grid, source, target: GridPos): float =
  discard g
  euclid(source, target)

proc reconstruct*[Node](cameFrom: Table[Node, Node], cur: Node): seq[Node] =
  result.add cur
  var c = cur
  while c in cameFrom:
    c = cameFrom[c]
...
proc aStar*(g: Grid, start, goal: GridPos, diag: bool = true): seq[GridPos] =
  ...
  open.push AStarNode(pos: start, g: 0.0, f: euclid(start, goal))
  var cameFrom = initTable[GridPos, GridPos]()
  var gScore = initTable[GridPos, float]()
  gScore[start] = 0.0
  var closed = initHashSet[GridPos]()

  template relax(nb: GridPos, stepCost: float) =
    let tentative = gScore.getOrDefault(cur, Inf) + stepCost
  ...
  while open.len > 0:
    ...
    if diag:
      for nb in neighbors8(g, cur):
        let stepCost = if abs(nb.x - cur.x) + abs(nb.y - cur.y) == 2: Sqrt2 else: 1.0
        relax(nb, stepCost)
    else:
      for nb in neighbors4(g, cur):
        relax(nb, 1.0)
...
proc aStarGeneric*[Node, G](
  ...
  ## Find a lowest-cost path using compile-time checked graph operations.
  when not compiles(block:
    var n: Node
    var s: seq[Node] = graph.neighbors(n)
    discard s
  ):
    {.error: "Graph must provide `neighbors(graph, Node): seq[Node]`.".}
  when not compiles(block:
    var n: Node
    var c: float = graph.cost(n, n)
    discard c
  ):
    {.error: "Graph must provide `cost(graph, Node, Node): float`.".}
  when not compiles(block:
    var n: Node
    var h: float = graph.heuristic(n, n)
    discard h
  ):
    {.error: "Graph must provide `heuristic(graph, Node, Node): float`.".}
...
when isMainModule:
  ...
  for row in viz:
    echo row.join("")
  echo "\nWhy Nim? Generic A* can be written once and reused for grids, navmeshes, or game states. Iterators + heapqueue give zero-allocation neighbor generation."
