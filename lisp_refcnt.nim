# Modern Idiomatic Nim: Lisp with Reference Counting Memory Manager
...
proc initMemory() =
  for i in 1 .. MemSize:
    memory[i] = SExp(refCnt: 0, kind: skList, carVal: nil, cdrVal: nil)
  for i in 1 ..< MemSize:
...
proc decRef(s: SExp) =
  ...
  if s.refCnt <= 0:
    if s.kind == skList:
      decRef(s.carVal)
      decRef(s.cdrVal)
    s[] = SExp(refCnt: 0, kind: skList, carVal: freeSEXP, cdrVal: nil)[]
    freeSEXP = s
...
proc allocSExp(k: SExpKind): SExp =
  ...
  freeSEXP = s.carVal
  case k
  of skNil: s[] = SExp(refCnt: 0, kind: skNil)[]
  of skNum: s[] = SExp(refCnt: 0, kind: skNum, intVal: 0)[]
  of skSym: s[] = SExp(refCnt: 0, kind: skSym, symVal: "")[]
  of skList: s[] = SExp(refCnt: 0, kind: skList, carVal: nil, cdrVal: nil)[]
  s
...
proc parseExp(p: var Parser): Exp =
  ...
  of tkQuote:
    ...
    let s = p.parseSExp()
    incRef(s)
    Exp(kind: ekVal, sxp: s)
  of tkInt:
    discard p.next()
    let s = mkNum(tok.intVal)
    incRef(s)
    Exp(kind: ekVal, sxp: s)
  of tkSym:
    discard p.next()
...
proc eval(e: Exp) =
  ...
  of ekCall:
    ...
    of "set":
      ...
      eval(e.args[1])
      let val = evalStack[^1]
      if globalEnv.hasKey(e.args[0].name):
        decRef(globalEnv[e.args[0].name])
      incRef(val)
...
proc main() =
  ...
  trueValue = mkSym("T")
  incRef(trueValue)
  incRef(trueValue) # for globalEnv binding
