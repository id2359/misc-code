...
NB. Orloci ordination: returns 2-column projection matrix
NB. First axis = line through two most outlying points,
NB. Second axis = perpendicular through point furthest from first axis.
Orloci =: 3 : 0
  D =. sqDistMatrix y
  ij =. (# y) #: (i. >./) , D
  i =. 0 { ij
  j =. 1 { ij
  X =. y -"1 i { y                    NB. shift origin to point i
  Xj =. j { X                          NB. defines first axis
  A =. (+/"1 X *"1 Xj) % +/ Xj * Xj   NB. projection coefficients on axis ij
  R =. +/"1 *: X -"1 A *"0 1 Xj        NB. squared residuals
  k =. R i. >./ R                      NB. point furthest from line ij
  X =. X -"1 (k { A) * Xj              NB. orthogonal shift defining 2nd axis
  axis2 =. k { X
  X +/ . * |: (j { X) ,: axis2         NB. project onto two axes
)
